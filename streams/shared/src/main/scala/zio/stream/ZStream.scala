...
trait ZStream[-R, +E, +A] extends Serializable { self =>
  ...
  final def window2[B](f: (A, A) => B): ZStream[R, E, B] =
    new ZStream[R, E, B] {
      def fold[R1 <: R, E1 >: E, B1 >: B, S]: ZStream.Fold[R1,E1,B1,S] = foldDefault

      override def process: ZManaged[R, E, InputStream[E, B]] =
        self.toList.map { getList =>
          (for {
            list <- getList
            r    = list.coflatMap { co =>
              co.tail.map(h => f(h.head, co.head))
            }
            s  <- r.toStream
          } yield s).flatMap(_.flatten)
        }
    }

  // final def duplicate: ZManaged[R, E, InputStream[E, InputStream[E, A]]] =
  //   self.zipper.map { stream =>
  //     stream.map { co =>
  //       co.cojoin.coflatMap()
  //     }
  //   }
  ...
  final def zipper: ZManaged[R, E, InputStream[E, StreamZipper[E, A]]] =
    process.map { next =>
      next.map { start =>
        StreamZipper(start, next)
      }
    }

  final def toList: ZManaged[R, E, IO[Option[E], StreamList[E, A]]] =
    process.map { stream =>
      InputStream.toList(stream)
    }
  ...
  def persist: ZManaged[R, E, ZStream[R, E, A]] =
    self.toList.map { cached =>
      new ZStream[R, E, A] {
        def fold[R1 <: R, E1 >: E, A1 >: A, S]: ZStream.Fold[R1, E1, A1, S] = foldDefault
        override def process: ZManaged[R, E, InputStream[E,A]] =
          ZManaged.succeed(cached.flatMap(_.toStream.flatten))
      }
    }

  // def replicate(n: Int): ZManaged[R, Option[E], List[ZStream[R, E, A]]] =
  //   self.toList.mapError(Some(_)).flatMap(_.map { cached =>
  //     val newStream = () => new ZStream[R, E, A] {
  //       def fold[R1 <: R, E1 >: E, A1 >: A, S]: ZStream.Fold[R1, E1, A1, S] = foldDefault
  //         override def process: ZManaged[R,E,InputStream[E,A]] =
  //           ZManaged.succeed(cached.toStream)
  //     }
  //     List.fill(n)(newStream())
  //   }.toManaged_)
}
...
object ZStream extends ZStreamPlatformSpecific {
  ...
  object InputStream {
    ...
    def toList[E, A](next: InputStream[E, A]): IO[Option[E], StreamList[E, A]] = {
      def loop(head: A, level: Long, cache: RefM[Map[Long, Promise[Nothing, Exit[Option[E], A]]]]): StreamList[E, A] = {
        val tail = for {
          nextHead <- cache.modify { c =>
            println((c, level))
            c.get(level) match {
              case None =>
                ZIO.dieMessage("Invalid State.")
              case Some(p) =>
                for {
                  d  <- p.isDone
                  e  <- if (d) p.await
                        else ZIO.interruptibleMask { restore =>
                          restore(next.run).flatMap(p.succeed(_))
                        } *> p.await
                  n  <- ZIO.done(e)
                  c1 <- if (c.contains(level + 1)) ZIO.succeed(c)
                        else for {
                          p <- Promise.make[Nothing, Exit[Option[E], A]]
                        } yield c + ((level + 1) -> p)
                } yield (n, c1)
            }
          }
        } yield loop(nextHead, level + 1, cache)
        StreamList(head, tail)
      }
      for {
        head <- next
        p    <- Promise.make[Nothing, Exit[Option[E], A]]
        ref  <- RefM.make[Map[Long, Promise[Nothing, Exit[Option[E], A]]]](Map(0L -> p))
      } yield loop(head, 0L, ref)
    }
  }
