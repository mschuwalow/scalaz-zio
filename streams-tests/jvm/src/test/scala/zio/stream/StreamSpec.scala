...
class ZStreamSpec(implicit ee: org.specs2.concurrent.ExecutionEnv)
  ...
  Stream.groupBy
    ...
    inner errors                                          $groupByErrorsInner
  ...
  private def groupByValuesInner =
    unsafeRun {
      ...
      Stream
        ...
        .runCollect
        .map(_.toMap must_=== (0 to 2).map((_.toString -> 1000)).toMap)
    }
  ...
  private def groupByErrorsInner =
    unsafeRun {
      val words = List.fill(1)(0 to 100).flatten
      for {
        result <- (Stream.fromIterable(words) ++ Stream.fromEffect(ZIO.effectTotal({println("foo"); 1})))
                    .process.use { is =>
                      ZStream.InputStream.toList(is).flatMap { stream =>
                        stream.foldLeft(0){ case (i1, i2) => i1 + i2} *>
                        ZIO.effectTotal(println("first ended")) *>
                        stream.foldLeft(0){ case (i1, i2) => {println(i1); i1 + i2}}
                      }
                    }
        _ <- ZIO.effect(println(result))
      } yield true must_=== true
    }
